...
import contractFundServices from '~/services/contractFundServices';
...
function DetailContractFund({onClose, userContractFund}: PropsDetailContractFund) {
	...
	const {data: listContractFundDetail} = useQuery(
		[
			...
			userContractFund?.code,
		],
		{
			queryFn: () =>
				httpRequest({
					http: contractFundServices.listDetailContractContractFundReportOverview({
						page: Number(_page) || 1,
						pageSize: Number(_pageSize) || 100,
						contractUuid: userContractFund.contractUuid,
						projectUuid: userContractFund.projectUuid,
						month: userContractFund.monthForDetail,
						year: userContractFund.yearForDetail,
					}),
				}),
			select(data) {
				return data;
